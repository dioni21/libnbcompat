...
## Build SRPM from spec: libnbcompat.spec
...
copr-srpm: srpm
	...
	if $$COPR_CMD build "$(COPR_PROJECT)" "$$srpm" >$$tmp 2>&1; then \
		...
		if grep -q "does not exist" $$tmp; then \
			echo "Project $(COPR_PROJECT) does not exist on COPR. Create it or override COPR_PROJECT: make copr-srpm COPR_PROJECT=<owner>/libnbcompat"; \
		else \
